...
// async function swap_jup (connection:Connection, input:string, output:string, amount:number, payer:Keypair) {

//     const quoteResponse = await (
//         await fetch(`https://quote-api.jup.ag/v6/quote?inputMint=${input}\&outputMint=${output}\&amount=${amount}\&slippageBps=500`
//         )
//     ).json();

//     if(quoteResponse.error){throw new Error(`Couldn't Compute Route For Swap ${amount} ${input} to ${output}`)};
...
//                 wrapAndUnwrapSol: true,
//                 dynamicComputeUnitLimit: true, // allow dynamic compute limit instead of max 1,400,000
//                 prioritizationFeeLamports: 150000 // or custom lamports: 1000
//             })
//         })
//     ).json();
...
//     if (simulation.value.err) {throw new Error('Simulation Returned Error' + JSON.stringify(simulation.value.err))}
//     // Execute the transaction
//     const rawTransaction = transaction.serialize()
//     const txid = await connection.sendRawTransaction(rawTransaction, {
//         skipPreflight: true,
...
//     }
//     catch(e) {
//         throw new Error(`Transaction Confirmation Expired, please review priority fees`)
//     }
